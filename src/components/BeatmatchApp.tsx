...
import { BEAT_PATTERNS, type BeatPattern, useAudioEngine } from "@/components/audio/AudioEngine";
...
export const BeatmatchApp: React.FC = () => {
  ...
  const updateDeckTempo = (deck: 'A' | 'B') => {
    const jogWheelState = deck === 'A' ? jogWheelStateA.current : jogWheelStateB.current;
    const jogWheelPitchBend = jogWheelState.velocity * 8; // Same scaling as in the decay animation

    if (deck === 'A') {
      const totalTempo = gameState.deckATempo + gameState.deckATempoOffset + jogWheelPitchBend;
      deckA.setTempo(totalTempo);
    } else {
      const totalTempo = gameState.deckBTempo + gameState.deckBTempoOffset + jogWheelPitchBend;
      deckB.setTempo(totalTempo);
    }
  };

  const updateJogWheelVelocity = (deck: 'A' | 'B', inputVelocity: number) => {
    const state = deck === 'A' ? jogWheelStateA.current : jogWheelStateB.current;
    ...
    const decayAnimation = () => {
      ...
      if (timeSinceInput > 50) { // 50ms without input starts decay
      ...
      if (deck === 'A') {
        setGameState((prev) => ({ ...prev, deckAJogWheelVelocity: state.velocity }));
      } else {
        setGameState((prev) => ({ ...prev, deckBJogWheelVelocity: state.velocity }));
      }
